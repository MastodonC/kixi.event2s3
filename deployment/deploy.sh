...
VPC=sandpit
sed -e "s/@@TAG@@/$TAG/" -e "s/@@AWS_REGION@@/$AWS_REGION/" -e "s/@@S3_BUCKET@@/$S3_BUCKET/" -e "s/@@ENVIRONMENT@@/$ENVIRONMENT/" -e "s/@@VPC@@/$VPC/" -e "s/@@CANARY@@/$CANARY/" -e "s/@@APP_NAME@@/$APP_NAME/" -e "s/@@IMAGE_PREFIX@@/$IMAGE_PREFIX/"  -e "s/@@INSTANCE_COUNT@@/$INSTANCE_COUNT/" -e "s/@@SYSTEM_PROFILE@@/$SYSTEM_PROFILE/" -e "s/@@ONYX_BATCH_SIZE@@/$ONYX_BATCH_SIZE/" ./deployment/marathon-config.json.template > $APP_NAME.json
